...
-- SELECT store_name, goods_name, quantity FROM stock_table stock
--   JOIN store_table store ON stock.store_code = store.store_code
--   JOIN goods_table goods ON stock.goods_code = goods.goods_code
...
-- SELECT store_code FROM stock_table stock WHERE stock.quantity
--   > ANY(SELECT AVG(quantity) FROM stock_table WHERE store_code='EA01')
...
--   VALUES ('M001', 'マフラー', 4500, '2020-09-02')


-- 7. 在庫テーブルの商品コード='S987'、かつ、店舗コード='EA01'に対して、「在庫数=10、更新日付=本日日付」で更新しなさい。※実行後のSELECT結果も貼付すること。
-- UPDATE stock_table stock
-- SET
-- quantity = 10,
-- update_day = '2020-09-02'
-- WHERE stock.goods_code = 'S987' AND stock.store_code = 'EA01'


-- 8. 7で更新した商品を削除しなさい。※実行後のSELECT結果も貼付すること。
-- DELETE FROM stock_table stock WHERE stock.goods_code = 'S987' AND stock.store_code = 'EA01'